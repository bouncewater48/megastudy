package variable.test;

import java.util.Scanner;

public class Test03 {
    public static void main(String[] args) {
        Scanner scan = new Scanner(System.in);

/*
        1. 입력 출력
        나이를 입력받아서 아래와 같이 출력하세요.
        입력 예시

        나이를 입력하세요
        24
        출력 예시

        이름 : 김인규 나이 : 24
*/

        System.out.println("이름과 나이를 입력하세요");
        String name = scan.next();
        int age = scan.nextInt();

        System.out.println("이름 : " + name + ", 나이 : " + age);
//        System.out.println("이름 : 강승훈, 나이 : " + age);

/*
        2. 입력과 더하기
        두 수를 입력받아서 더하고 아래와 같이 출력하세요.
        입력 예시

        두 수를 입력하세요
        3 5
        출력 예시

        3 + 5 = 8
*/

        System.out.println("두 수를 입력하세요");
        int number1 = scan.nextInt();
        int number2 = scan.nextInt();

        System.out.println(number1 + " + " + number2 + " = " + (number1 + number2));

        /*
        * 3. 입력과 빼기
        두 수를 입력받아서 빼고 아래와 같이 출력하세요.
        입력 예시

        두 수를 입력하세요
        3 5
        출력 예시

        3 - 5 = -2
        */

        System.out.println("두 수를 입력하세요");
        System.out.println(number1 + " - " + number2 + " = " + (number1 - number2));

        /*
        4. 구구단
        단수를 입력받고 아래와 같이 구구단을 출력하세요.
        입력 예시

        단수를 입력하세요 : 5
        출력 예시

        5 X 1 = 5
        5 X 2 = 10
        5 X 3 = 15
        5 X 4 = 20
        5 X 5 = 25
        */

//        int gugudan;
        System.out.println("단수를 입력하세요");
        int gugudan = scan.nextInt();
//        System.out.println("단수를 입력하세요 : " + gugudan);

        System.out.println(gugudan + " X 1 = " + (gugudan * 1));
        System.out.println(gugudan + " X 2 = " + (gugudan * 2));
        System.out.println(gugudan + " X 3 = " + (gugudan * 3));
        System.out.println(gugudan + " X 4 = " + (gugudan * 4));
        System.out.println(gugudan + " X 5 = " + (gugudan * 5));

        /*
        5. 원 넓이 구하기
        지름을 입력받아서 원의 넓이를 구하여 출력하세요.
        pi는 3.14
        입력 예시

        지름을 입력하세요 : 24
        출력 예시

        원의 넓이는 452.16입니다.
        */

        System.out.println("지름을 입력하세요");
        int diameter = scan.nextInt();
//        System.out.println("지름을 입력하세요 : " + diameter);
        double pi = 3.14;
        int diameter2 = diameter/2 * diameter/2;
        System.out.println("원의 넓이는 " + (diameter2 *  pi) + "입니다.");
    }
}
